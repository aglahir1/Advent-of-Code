...
regex = re.compile(r'^(\D*) (\d{1,3}),(\d{1,3}) through (\d{1,3}),(\d{1,3})$')

i = """turn on 489,959 through 759,964
...
def toggle(a):
    for light in a:
        lights[light[0]][light[1]] = 1 - lights[light[0]][light[1]]

for entry in inputArray:
    match = regex.match(entry)
    command = match.group(1)
    coordinate1 = [int(match.group(2)),int(match.group(3))]
    coordinate2 = [int(match.group(4)),int(match.group(5))]
    change = affectedArray(coordinate1, coordinate2)
    if(command == 'turn on'): turnOn(change)
    if(command == 'turn off'): turnOff(change)
    if(command == 'toggle'): toggle(change)


amount = sum([sum(a) for a in lights])
print(amount)
